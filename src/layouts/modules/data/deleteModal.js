...
function AlertDialog({ name, restore, id, setData, parent_id }) {
  ...
  const handleDelete = () => {
    let url = parent_id === "" ? "module/delete/" + id : "course/delete/" + parent_id + "/" + id;
    let loader = parent_id === "" ? "modules/manage" : "course/modules/" + parent_id;
    // switch (name) {
    //   case "program":
    ...
    const fetchData = async () => {
      try {
        ...
        setData(parent_id != "" ? data.course : data);
        console.log(data);
      } catch (error) {
        console.error("Failed to fetch objects:", error);
      }
