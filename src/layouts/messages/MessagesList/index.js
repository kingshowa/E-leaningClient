...
function MessagesList({ patnerId, messages, shadow }) {
  const renderMessages = messages.map(({ id, text, attachment, sent_by, total_likes }) => (
    <MDBox key={id} component="li" display="flex" alignItems="center" py={1} mb={1}>
      ...
      >
        <MessageMenu
          msId={id}
          sender={{ name: null, surname: null }}
          toDelete={sent_by != patnerId}
        ...
        {total_likes > 0 && (
          <MDBox sx={{ mt: 4, ml: -2, mb: -2 }} minWidth={30}>
            <MDTypography variant="" color={"primary"}>
              <Icon>favorite</Icon>
            </MDTypography>
          </MDBox>
        )}
...
MessagesList.propTypes = {
  patnerId: PropTypes.number.isRequired,
